...
[delayed_sum, peak_sum] = formBeam(out, decimationRatio, delay_phases, 100,100,100,100, calAnt2, calAnt3, calAnt4);
...
if plotFFT == true  
    ...
    hold on;
    peak_sum_taper = peak_sum;
    taperPlot = plot(steer_angles, peak_sum_taper);
    taperPlot.XDataSource = 'steer_angles';
    taperPlot.YDataSource = 'peak_sum_taper';
    legend('No Taper', 'Taper');
    for i=1:500
        [out, valid] = rx();
        Gain1 = 30;
        Gain2 = 100;
        Gain3 = 100;
        Gain4 = 30;
        [delayed_sum, peak_sum_taper] = formBeam(out, decimationRatio, delay_phases, Gain1, Gain2, Gain3, Gain4, calAnt2, calAnt3, calAnt4);
        [delayed_sum, peak_sum] = formBeam(out, decimationRatio, delay_phases, 100,100,100,100, calAnt2, calAnt3, calAnt4);
        refreshdata
        drawnow
        pause(0.1);
...
function [delayed_sum, peak_sum] = formBeam(data, dec, delays, Gain1, Gain2, Gain3, Gain4, calAnt2, calAnt3, calAnt4)
    ...
    % Apply Gains from 0-100% on each element
    Ant1 = Ant1 * min(100, max(Gain1, 0))/100;
    Ant2 = Ant2 * min(100, max(Gain2, 0))/100;
    Ant3 = Ant3 * min(100, max(Gain3, 0))/100;
    Ant4 = Ant4 * min(100, max(Gain4, 0))/100;
